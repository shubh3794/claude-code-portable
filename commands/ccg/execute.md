...
根据计划中的"关键文件"列表，调用 `Glob + Grep`（MCP 未配置） 检索相关代码：
...
1. 调用 Gemini（使用 `/Users/shaggarw/.claude/.ccg/prompts/gemini/frontend.md`）
...
1. 调用 Codex（使用 `/Users/shaggarw/.claude/.ccg/prompts/codex/architect.md`）
2. 输入：计划内容 + 检索到的上下文 + 目标文件
3. OUTPUT: `Unified Diff Patch ONLY. Strictly prohibit any actual modifications.`
