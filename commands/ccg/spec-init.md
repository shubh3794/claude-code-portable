...
4. **Validate Multi-Model MCP Tools**
   - Check `codeagent-wrapper` availability: `/Users/shaggarw/.claude/bin/codeagent-wrapper --version`
   - **工作目录**：`{{WORKDIR}}` **必须通过 Bash 执行 `pwd`（Unix）或 `cd`（Windows CMD）获取当前工作目录的绝对路径**，禁止从 `$HOME` 或环境变量推断。如果用户通过 `/add-dir` 添加了多个工作区，先确定任务相关的工作区。
   - Test Codex backend:
     ```bash
     echo "echo test" | /Users/shaggarw/.claude/bin/codeagent-wrapper --backend codex - "{{WORKDIR}}"
     ```
   - Test Gemini backend:
     ```bash
     echo "echo test" | /Users/shaggarw/.claude/bin/codeagent-wrapper --backend gemini --gemini-model gemini-3.1-pro-preview - "{{WORKDIR}}"
     ```
