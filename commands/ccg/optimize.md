...
| 模型 | 提示词 |
|------|--------|
| Codex | `/Users/shaggarw/.claude/.ccg/prompts/codex/optimizer.md` |
| Gemini | `/Users/shaggarw/.claude/.ccg/prompts/gemini/optimizer.md` |
...
1. 调用 `Glob + Grep`（MCP 未配置） 检索目标代码（如可用）
2. 识别性能关键路径
3. 收集现有指标（如有）
...
1. **Codex 后端分析**：`Bash({ command: "...--backend codex...", run_in_background: true })`
   - ROLE_FILE: `/Users/shaggarw/.claude/.ccg/prompts/codex/optimizer.md`
   - 需求：分析后端性能问题（$ARGUMENTS）
   - OUTPUT：性能瓶颈列表、优化方案、预期收益

2. **Gemini 前端分析**：`Bash({ command: "...--backend gemini...", run_in_background: true })`
   - ROLE_FILE: `/Users/shaggarw/.claude/.ccg/prompts/gemini/optimizer.md`
