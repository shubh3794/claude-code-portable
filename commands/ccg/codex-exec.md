...
```
Bash({
  command: "/Users/shaggarw/.claude/bin/codeagent-wrapper --progress --backend codex - \"{{WORKDIR}}\" <<'EXEC_EOF'
<TASK>
<指令内容>
</TASK>
EXEC_EOF",
...
```
Bash({
  command: "/Users/shaggarw/.claude/bin/codeagent-wrapper --progress --backend codex resume <SESSION_ID> - \"{{WORKDIR}}\" <<'EXEC_EOF'
<TASK>
<指令内容>
</TASK>
EXEC_EOF",
...
```
Bash({
  command: "/Users/shaggarw/.claude/bin/codeagent-wrapper --progress --backend <codex|gemini> {{GEMINI_MODEL_FLAG}}- \"{{WORKDIR}}\" <<'REVIEW_EOF'
ROLE_FILE: <角色提示词路径>
<TASK>
...
```
Bash({
  command: "/Users/shaggarw/.claude/bin/codeagent-wrapper --progress --backend codex resume <CODEX_SESSION> - \"{{WORKDIR}}\" <<'EXEC_EOF'
<TASK>
...
```
Bash({
  command: "/Users/shaggarw/.claude/bin/codeagent-wrapper --progress --backend codex resume <CODEX_EXEC_SESSION> - \"{{WORKDIR}}\" <<'FIXEOF'
<TASK>
...
2. **并行调用**（`run_in_background: true`）：
   ...
   - **Codex 审查**：
     - ROLE_FILE: `/Users/shaggarw/.claude/.ccg/prompts/codex/reviewer.md`
     - 输入：变更 Diff + 计划文件内容
     - 关注：安全性、性能、错误处理、逻辑正确性

   - **Gemini 审查**：
     - ROLE_FILE: `/Users/shaggarw/.claude/.ccg/prompts/gemini/reviewer.md`
     - 输入：变更 Diff + 计划文件内容
     - 关注：代码可读性、设计一致性、可维护性
