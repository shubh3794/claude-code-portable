...
# 新会话调用
Bash({
  command: "/Users/shaggarw/.claude/bin/codeagent-wrapper --progress --backend <codex|gemini> {{GEMINI_MODEL_FLAG}}- \"{{WORKDIR}}\" <<'EOF'
ROLE_FILE: <角色提示词路径>
<TASK>
...
# 复用会话调用
Bash({
  command: "/Users/shaggarw/.claude/bin/codeagent-wrapper --progress --backend <codex|gemini> {{GEMINI_MODEL_FLAG}}resume <SESSION_ID> - \"{{WORKDIR}}\" <<'EOF'
ROLE_FILE: <角色提示词路径>
<TASK>
需求：<增强后的需求（如未增强则用 $ARGUMENTS）>
上下文：<前序阶段收集的项目上下文、计划文件内容等>
...
| 阶段 | Codex | Gemini |
|------|-------|--------|
| 分析 | `/Users/shaggarw/.claude/.ccg/prompts/codex/analyzer.md` | `/Users/shaggarw/.claude/.ccg/prompts/gemini/analyzer.md` |
| 规划 | `/Users/shaggarw/.claude/.ccg/prompts/codex/architect.md` | `/Users/shaggarw/.claude/.ccg/prompts/gemini/architect.md` |
| 实施 | `/Users/shaggarw/.claude/.ccg/prompts/codex/architect.md` | `/Users/shaggarw/.claude/.ccg/prompts/gemini/frontend.md` |
| 审查 | `/Users/shaggarw/.claude/.ccg/prompts/codex/reviewer.md` | `/Users/shaggarw/.claude/.ccg/prompts/gemini/reviewer.md` |
...
调用 `Glob + Grep`（MCP 未配置） 检索相关代码、组件、技术栈。
...
```
执行 agent: /Users/shaggarw/.claude/agents/ccg/ui-ux-designer.md
输入: 项目上下文 + 用户需求 + 技术栈
输出: UI/UX 设计方案
```
...
```
执行 agent: /Users/shaggarw/.claude/agents/ccg/planner.md
